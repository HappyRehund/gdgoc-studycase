...
function History({ userSettings }: { userSettings: UserSettings }) {
  ...
  const historyDataQuery = useQuery<GetHistoryDataResponseType>({
    queryKey: ["overview", "history", timeframe, period.year, period.month],
    queryFn: () =>
      fetch(
        ...
        <CardHeader className="gap-2">
          <CardTitle className="grid grid-flow-row justify-between gap-2 md:grid-flow-col">
            ...
            <div className="flex h-10 gap-2">
              <Badge
                variant={"outline"}
                className="text-cardcolor flex items-center gap-2 text-sm"
              >
...
function CustomTooltip({ active, payload, formatter }: any) {
  ...
  return (
    <div className="min-w-72 rounded border bg-cardbg p-4">
      <TooltipRow
        formatter={formatter}
        label="Expense"
        value={expense}
